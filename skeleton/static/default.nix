...
in pkgs.stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    # mkdir -p $out/md-out
    # cp -r md-src/* $out/md-out/

    # # file_contents=$(cat $out/md-out/chapter0.md)
    # #echo $file_contents

    # # iconv -t utf-8 $out/md-out/chapter0.md > $out/md-out/chapter0.md

    # mkdir -p "$out/html/landing/Blog"

    # for file in md-src/*.md; do
    ...
    #     iconv -t utf-8 -c md-src/$filename > $out/html/landing/Blog/$filename
    #     echo $out/html/Blog/$filename
    #     echo $out/html/Blog/$filename.html

    #     basename=$(basename "$filename" | cut -f 1 -d '.')
    #     echo "$basename"
 
    #     ${mmark-cli}/bin/mmark --ext-ghc-highlighter -i  $out/html/landing/Blog/$filename -o $out/html/landing/Blog/$basename.md.html
