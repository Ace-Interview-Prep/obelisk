...

{
  ...
  obelisk-command = haskellLib.overrideCabal
    (haskellLib.generateOptparseApplicativeCompletion "jenga"
      (haskellLib.justStaticExecutables super.obelisk-command))
    (drv: {
      buildTools = (drv.buildTools or []) ++ [ pkgs.buildPackages.makeWrapper ];
      postFixup = ''
        ${drv.postFixup or ""}
        # Make `jenga` reference its runtime dependencies.
        wrapProgram "$out"/bin/jenga --prefix PATH : ${pkgs.lib.makeBinPath commandRuntimeDeps}
      '';
