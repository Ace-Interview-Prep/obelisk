...
* Custom Setup.hs hooks (`jenga-setup`) handle cross-compilation of the
...
* New `nix/module.nix` module with declarative options:
  * `jenga.static.path` / `jenga.static.compress` / `jenga.static.compressed`: static asset pipeline with cache-busting hashes, brotli + gzip compression
  * `jenga.frontend.target`: select `"js"` (GHCJS) or `"wasm"` (default) frontend compilation target
  * `jenga.frontend.js.{package,optimization,optimized,compress,compressed}`: GHCJS pipeline with closure-compiler (ADVANCED mode by default)
  * `jenga.frontend.wasm.{package,optimization,optimized,compress,compressed}`: WASM pipeline with wasm-opt + wasm-tools strip
...
* New `jenga-setup` package with reusable Setup.hs hooks:
  * `Jenga.Setup.Backend`: symlinks frontend assets into backend data dir
  * `Jenga.Setup.Frontend.Js`: GHCJS cross-compilation with optimization forwarding
  * `Jenga.Setup.Frontend.Wasm`: WASM cross-compilation with jsexe assembly, JSFFI extraction, optional wasm-opt/wasm-tools
  * `Jenga.Setup.Static`: static manifest generation via `jenga-asset-manifest-generate`
  * `Jenga.Setup.Utils`: shared utilities (project root discovery, symlinks, cabal-level optimization flags)

### Static asset generation

* Replace hackage overlay approach with `jenga-generated-static` / `jenga-generated-static-custom` package pair:
  * `jenga-generated-static` (`build-type: Simple`) builds on all platforms including JS/WASM
  * `jenga-generated-static-custom` (`build-type: Custom`) runs Setup.hs to generate `Jenga.Generated.Static` module, `buildable: False` on JS/WASM
* `jenga-asset-manifest-generate --module-only` generates only the Haskell module without overwriting the `.cabal` file
...
* `ob-run`: watch-and-rebuild development server with inotifywait, forwards optimization level to cross-builds via `JENGA_CROSS_CABAL_ARGS`, proper cleanup of all child processes on exit
...
* `nix/server.nix`: NixOS module (`services.jenga`) with nginx reverse proxy (WebSocket support), ACME/HTTPS, systemd service, firewall rules, domain redirects
...
* `skeleton/`: minimal Jenga project template with complete directory structure:
  * backend, frontend, common, static packages
  * frontend-js and frontend-wasm cross-compilation wrappers
  * `project.nix` with `source-repository-packages` from Jenga, cross-platform shell (ghcjs + wasi32), hoogle
...
* [#1038](https://github.com/obsidiansystems/obelisk/pull/1038): `Jenga.Route`: Add `pathQueryEncoder` and `generalizeIdentity`
...
* [#1099](https://github.com/obsidiansystems/obelisk/pull/1099): `Jenga.Route`: Add `pairRoute` and deprecate `subPairRoute` and `subPairRoute_`
...
* Documentation
  * [#913](https://github.com/obsidiansystems/obelisk/pull/913): Add haddocks to `Jenga.Command.Deploy`
  * [#919](https://github.com/obsidiansystems/obelisk/pull/919): Document useful command for testing Jenga branches to CONTRIBUTING.md
...
* CLI
  ...
  * [#934](https://github.com/obsidiansystems/obelisk/pull/934), [#936](https://github.com/obsidiansystems/obelisk/pull/936): Jenga now always uses absolute paths when generating `.ghci` files for REPL and IDE support. This is a **BREAKING** change for some old tools that could not handle absolute paths properly. However, due to the behavior of cross-volume relative paths on certain platforms, such as modern MacOS, we cannot guarantee proper operation of Jenga with relative paths.
  * [#948](https://github.com/obsidiansystems/obelisk/pull/948): Jenga now always invokes `bash` instead of the system-wide shell when it can. Some sub-programs, like `ghcid`, will still use the system-wide shell, which can cause subtle problems due to impurities.
  * [#949](https://github.com/obsidiansystems/obelisk/pull/949): Jenga now configures its output and error streams to transliterate unsupported characters to a known-good replacement character.
...
* jenga-route
  * [#915](https://github.com/obsidiansystems/obelisk/pull/915): Add `routeLinkAttr` to `Jenga.Route.Frontend`. This allows the creation of route links with additional, user-specified DOM attributes.
  * [#918](https://github.com/obsidiansystems/obelisk/pull/918): Add GHC 8.10.7 support for `jenga-route`.
...
* Update reflex-platform to v0.9.2.0
  * This updated reflex-dom-core to [0.7](https://github.com/reflex-frp/reflex-dom/releases/tag/reflex-dom-core-0.7.0.0), which removes the `js` type parameter from `Prerender` (i.e., `Prerender js t m` becomes `Prerender t m`) and removes `HasJS` and `HasJSContext`. This resulted in changes to the following Jenga modules:
    * `Jenga.Configs`: `HasJSContext` and `HasJS` are no longer derived.
    * `Jenga.Frontend`: `JengaWidget js t route m` no longer has the `js` type parameter. It is now `JengaWidget t route m`.
    * `Jenga.Route.Frontend`: There are no longer `HasJSContext` or `HasJS` instances for `RoutedT`, `SetRouteT`, `RouteToUrlT`.
...
* Use TemplateHaskell to determine asset file paths
  ...
  * Deprecation: Deprecate static asset modules generated via 'jenga-asset-manifest-generate' in favor of modules generated via 'jenga-asset-th-generate'. The new executable takes the same arguments as the old and should be a drop-in replacement. To preserve the old behavior, set `__deprecated.useJengaAssetManifestGenerate = true;` in your Jenga project configuration.
...
* Feature: Add `staticFilePath` to `Jenga.Generated.Static`. Like `static`, this uses TH to generate a reference to a file. Unlike `static`, this `staticFilePath` generates a path on the filesystem instead of URL path.
* Docs: Added [documentation](lib/route/docs/introduction.md) for jenga-routes.
...
* **(Breaking change)** Backport nixpkgs upgrades to ACME/Let's Encrypt handling so that HTTPS deployments continue to work flawlessly. If your deployment is having trouble renewing [Let's Encrypt](https://letsencrypt.org/) certificates, upgrade to this version.
  * **IMPORTANT:** In order to use [Let's Encrypt](https://letsencrypt.org/) you must now accept their [terms of service](https://letsencrypt.org/repository/). To do that, add `terms.security.acme.acceptTerms = true;` to the `import ./.jenga/impl {` section in your `default.nix`. The new skeleton application may serve as an [example](https://github.com/obsidiansystems/obelisk/blob/4759342ab3570888c027d4c58cb5694cb832d624/skeleton/default.nix#L13).
* Update reflex-platform dependency to v0.6.0.0
* ([#715](https://github.com/obsidiansystems/obelisk/pull/715)) In `Jenga.Route` deprecate `isoEncoder` and `prismEncoder` in favor of more precisely named `viewEncoder` and `reviewEncoder` (respectively) and improve documentation regarding contravariance of `reviewEncoder`.
...
* ([#742](https://github.com/obsidiansystems/obelisk/pull/742), [#57](https://github.com/obsidiansystems/obelisk/pull/57), [#406](https://github.com/obsidiansystems/obelisk/pull/406)) Enable `-dedupe` and `-DGHCJS_BROWSER` in GHCJS builds to make JavaScript output considerably smaller leading to faster load/parse times and faster build times.
  * **Migration:** New Jenga projects will automatically benefit from this change, but existing projects need to apply a change similar to [this one](https://github.com/obsidiansystems/obelisk/blob/371cb3302085601c5ec73e9574d51c8b95e3e493/skeleton/frontend/frontend.cabal#L32-L34).
...
* ([#695](https://github.com/obsidiansystems/obelisk/pull/695)) `ob deploy init` now requires that your Jenga project be a clean `git` checkout with pushed changes. It has always required that your Jenga project be a `git` repository, but it did not require that your local changes be committed and pushed. This new requirement is added to ensure users don't accidentally create a deployment pointing to an old version of the project.
* ([#705](https://github.com/obsidiansystems/obelisk/pull/705)) Add `Jenga.Route.packTextEncoder` and generalize `Jenga.Route.unpackTextEncoder` over any `Data.Text.Lens.IsText`.
...
* ([#700](https://github.com/obsidiansystems/obelisk/pull/700)) Ensure `ob init` uses the thunk format of the target Jenga rather than the one currently running the init command. If a user had installed a version of Jenga with a new thunk format, a newly initialized skeleton of an older version would not be able to unpack its own `.jenga/impl`.
...
* ([#709](https://github.com/obsidiansystems/obelisk/pull/709)) Fix a bug in Jenga's preprocessor causing it to incorrectly skip files in some circumstances.
...
* Add `Jenga.Backend.runBackendWith` to allow several customizations. ([#668](https://github.com/obsidiansystems/obelisk/pull/668), [#644](https://github.com/obsidiansystems/obelisk/pull/644))
* Add `ob profile` command to run Jenga projects with profiling. `ob profile` works like `ob run`, but instead of using `ghci`, it builds an executable that is built with profiling enabled. ([#654](https://github.com/obsidiansystems/obelisk/pull/654))
* Jenga's `default.nix` now exposes `mkAssets` function which is used to construct the assets served by a Jenga application. ([#651](https://github.com/obsidiansystems/obelisk/pull/651))
* Bump reflex-platform to v0.5.2.0. ([#671](https://github.com/obsidiansystems/obelisk/pull/671))

## v0.6.0.0 - 2020-02-21

* Fix a bug in `Jenga.Route.Frontend` where `routeLink`, `routeLinkDynAttr`, and `dynRouteLink` would not behave exactly like `<a href="...">` when run by JavaScript. These functions now scroll to the top of the page when the link is clicked. ([#540](https://github.com/obsidiansystems/obelisk/pull/540))
* Fix a bug in `ob run`/`ob watch`/`ob repl` where nested Jenga projects would also be loaded into the session. These are now ignored. ([#652](https://github.com/obsidiansystems/obelisk/pull/652))
* Improve behavior of `ob run`/`ob watch`/`ob repl` when multiple packages with the same name are encountered. Jenga now issues a warning and tells you which one it will use. ([#653](https://github.com/obsidiansystems/obelisk/pull/653))
* Removed `Jenga.Backend.mkRouteToUrl` since it is easily written in terms of `Jenga.Route.renderJengaRoute`:

      mkRouteToUrl validFullEncoder (k :/ v) = renderJengaRoute validFullEncoder (FullRoute_Frontend (JengaRoute_App k) :/ v)

* Add `Jenga.Backend.renderAllJsPath` to expose URL path to `ghcjs/all.js`. ([#545](https://github.com/obsidiansystems/obelisk/pull/545))
* Add argument to `serveDefaultJengaApp`, `serveJengaApp`, and `serveGhcjsApp` to take the path to `all.js` instead of hard-coding it. ([#545](https://github.com/obsidiansystems/obelisk/pull/545))

## v0.5.0.0 - 2020-02-07

* Add `Jenga.Route.(?/)`, a convenience function for constructing routes nested in `Maybe`. ([#457](https://github.com/obsidiansystems/obelisk/pull/457))
* Add local unpacked packages to the `ob run`, `ob watch`, and `ob repl` sessions. Any `.cabal` or hpack package inside the current Jenga project will be loaded into the session. For `ob run`/`ob watch` this means the session will automatically reload when you save a source file in any of those packages. For `ob repl` it means that `:r` will reload changes to any of those packages. There are some edge cases where this integration is still rough. Report any issues you encounter. ([#489](https://github.com/obsidiansystems/obelisk/pull/489))
...
* Fix `ob` commands to again support running from any subdirectory of a Jenga project ([#591](https://github.com/obsidiansystems/obelisk/pull/591))
* Add `reflex-platform-func` argument to Jenga's `default.nix`. It defaults to its prior behavior of using the reflex-platform in in `dep`. ([#612](https://github.com/obsidiansystems/obelisk/pull/612))

## v0.3.0.0 - 2019-12-20

* Change the structure of Jenga routes to use a designated
  `FullRoute` type. This combines frontend and backend routes into one
  structure. This is a **breaking** change which requires Jenga apps
...
* Bump reflex-platform so that Jenga now uses GHC 8.6.5 and the nixos-19.03 nixpkgs set.
* Add support in `jenga-route` for single parameters in URL paths.
* Bump reflex-platform so that Jenga now uses reflex-dom 0.5.2.0.
* Use a `--pure` nix shell in `ob run` for parity with `ob repl` and more resilience against "works on my machine".
* Pin usages of `<nixpkgs>` in Jenga thunks, etc. to the nixpkgs used by the project's Jenga impl.
* Backport ACMEv2 support in Jenga server to regain LetsEncrypt account creation.
...
* Add `Jenga.Route.Frontend.routeLinkDynAttr`.
...
* FrontendConfigsT has been changed into ConfigsT and configs are made available via getConfig/getConfigs
  * The frontend will still only have access to configs that are placed in config/frontend and config/common, while the backend has access to the entire contents of the config directory via `Jenga.ExecutableConfig.Lookup.getConfigs`.
...
* Provide a way to indicate acceptance of the Android SDK license by passing `config.android_sdk.accept_license = true;` in the arguments to the import of `.jenga/impl` in the project's `default.nix`.
...
* Make asset path hashing strict (see `Jenga.Asset.Gather`)
* Add the `ob shell` command to enter a nix shell for a Jenga project
* Allow skeleton's Jenga to be overridden. This changes the skeleton's default.nix interface: the arguments that it used to take are now part of the new "jenga" argument.
* Removed `MonadIO` from `JengaWidget` to prevent accidental IO during prerendering. If you need to do IO in a widget it should be on the right hand side of a `prerender`.
* Significantly changed the interface to the "executable config" packages. `jenga-executable-config-lookup` is a new internal package which looks up all configs in a platform-specific way. `jenga-executable-frontend` and `jenga-executable-backend` provide MTL-style monad classes (`HasFrontendConfigs` and `HasBackendConfigs`) which the frontend and backend, respectively, can use to look up configs. This replaces the old `get` function which ran in `IO`.
* Add a flag to force thunk packing even if there are unpushed changes in the unpacked thunk.


## v0.1.0.0 - 2019-03-29

* Use reflex-dom's `HydrationDomBuilder` to "hydrate" statically rendered DOM served by the Jenga backend (rather than re-creating DOM and replacing it all).
* Add `HasCookies` and `CookiesT` to allow `JengaWidget`s to access cookies during static and "hydrated" DOM rendering.
