...
module Jenga.Setup.Frontend.Wasm (main) where
...
import Jenga.Setup.Utils (crossCabalArgs, findProjectRoot, optLevelFlags, strip, symlink)
...
copyWasiShim jsexeDir = do
  shimPathM <- lookupEnv "JENGA_WASI_SHIM"
  shimPath <- case shimPathM of
    Nothing -> fail "[Setup] JENGA_WASI_SHIM not set. Set it to the @bjorn3/browser_wasi_shim package root."
    Just p -> pure p
  let distDir = shimPath </> "dist"
  distExists <- doesDirectoryExist distDir
  unless distExists $
    fail $ "[Setup] JENGA_WASI_SHIM dist directory not found: " <> distDir
