...
-- | Shared utilities for Jenga Setup.hs hooks.
module Jenga.Setup.Utils
  ( findProjectRoot
  , symlink
  , crossCabalArgs
  , optLevelFlags
...
-- | Read extra cabal arguments for cross builds from @JENGA_CROSS_CABAL_ARGS@.
crossCabalArgs :: IO [String]
crossCabalArgs = maybe [] words <$> lookupEnv "JENGA_CROSS_CABAL_ARGS"
