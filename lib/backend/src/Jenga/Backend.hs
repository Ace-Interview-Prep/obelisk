...
import Jenga.Asset.Serve.Snap (serveAsset)
import qualified Jenga.ExecutableConfig.Lookup as Lookup
import Jenga.Frontend
import Jenga.Route
import Jenga.Snap.Extras (doNotCache, serveFileIfExistsAs)
...
-- | Serve a frontend, which must be the same frontend that Jenga has built and placed in the default location
...
serveDefaultJengaApp
  ...
  -> Frontend (R appRoute)
  -> Map Text ByteString
  -> R (JengaRoute appRoute)
  -> m ()
serveDefaultJengaApp urlEnc ghcjsWidgets serveStaticAsset frontend =
  serveJengaApp urlEnc ghcjsWidgets serveStaticAsset frontendApp
...
renderAllJsPath validFullEncoder =
  renderJengaRoute validFullEncoder $ FullRoute_Frontend (JengaRoute_Resource ResourceRoute_Ghcjs) :/ ["all.js"]
...
serveJengaApp
  ...
  -> R (JengaRoute appRoute)
  -> m ()
serveJengaApp urlEnc ghcjsWidgets serveStaticAsset frontendApp config = \case
  JengaRoute_App appRouteComponent :=> Identity appRouteRest -> serveGhcjsApp urlEnc ghcjsWidgets frontendApp config $ GhcjsAppRoute_App appRouteComponent :/ appRouteRest
  JengaRoute_Resource resComponent :=> Identity resRest -> case resComponent :=> Identity resRest of
    ...
    ResourceRoute_JSaddleWarp :=> Identity _ -> do
      let msg = "Error: Jenga.Backend received jsaddle request"
...
-- | Default Jenga backend configuration.
      ...
      runSnap $
        getRouteWith validFullEncoder >>= \case
          Identity r -> case r of
            ...
            FullRoute_Frontend jengaRoute :/ a ->
              serveJengaApp routeToUrl (($ allJsUrl) <$> ghcjsWidgets) (serveStaticAssets staticAssets) frontendApp publicConfigs $
                jengaRoute :/ a
              where
                routeToUrl (k :/ v) = renderJengaRoute validFullEncoder $ FullRoute_Frontend (JengaRoute_App k) :/ v
...
-- | Get configs from the canonical "public" locations (i.e., locations that Jenga expects to make available
