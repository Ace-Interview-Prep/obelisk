...
-- | Apply a parser as a search-and-replace over the input string.
-- Every match is transformed by the given function; non-matching
-- characters pass through unchanged. Should never return 'Left'.
--
-- @since 0.1.0.0
...
-- | Stream-editing parser: matches are transformed via the supplied
-- function, non-matching characters are carried through, and the
-- result is the fully reconstructed output string.
--
-- @since 0.1.0.0
...
-- | Like 'findEdit' but only replaces the first match. After the first
-- match is transformed, all remaining input is consumed verbatim.
--
-- @since 0.1.0.0
...
editFirst f parser = do
  ...
  x <- ((Edit . f) <$> (try parser)) <|> (Carry <$> anyChar) <|> endStream'
  case x of
    Edit str -> fmap (str <>) $ many anyChar
    Carry chr -> fmap ([chr] <>) (findEdit f parser)
    EOF -> return []
