...
import Prelude (Bool(..), Either(..), Eq(..), Int, Maybe(..), Ord(..), Read, Show, String,
                ($), (&&), (<$>), (<*>), (<>), (.), (==), (/=), drop, elem, error, fmap, flip, last, length,
                not, otherwise, return, snd, fst, undefined)
...
fixRelativeUrl (Link bUrl) url
  ...
  | last bUrl /= '/' && (not $ isPrefixOf "/" url) = bUrl <> "/" <> url
  | otherwise = error "fixRelativeUrl: unreachable"
...
urlIsNew (branch:tree) uri
  | eq1 (fmap URI.uriPath (mkURI' (uri))) (fmap URI.uriPath (mkURI' (Prelude.snd branch))) = False
...
maybeNewUrl (branch:tree) uri =
  if eq1 (fmap URI.uriPath (mkURI' (renderLink uri))) (fmap URI.uriPath (mkURI' . renderLink . Prelude.fst $ branch))
  then Nothing
...
maybeUsefulUrl (Link baseUrl) url = do
  ...
  where
    ...
    allowableEndings url' =
      ...
      in
        if (elem '.' lastPath)
        then allowableFile lastPath url'
        else Just url'
...
getLastPath (Link url) = do
  ...
  x' <- URI.uriPath x
  Just . T.unpack . URI.unRText . NE.last . Prelude.snd $ x'
