...
import Prelude
  ( Bool(..), Char, Either(..), Maybe(..), String
  , ($), (.), (<>), (&&), (==), (/=), (-), (||), (>>)
  , fmap, return
  , elem, fst, length, otherwise, show, snd, undefined
  )

import Scrappy.Links (Link, LastUrl, CurrentUrl)
import Scrappy.Elem.Types (Elem, ElemHead, Attrs, AttrsError(IncorrectAttrs), getHrefAttrs)
...
parseAttrSafe attrName' = do
  tag <- parseOpeningTag Nothing [(attrName', Nothing)]
...
isAttrsMatch mapAttr ((name, maybeVal): desired) = case maybeVal of
  ...
  Nothing ->
    case Map.lookup name mapAttr of
      Just _irrValFromKey -> isAttrsMatch mapAttr desired
      Nothing -> False
...
parseOpeningTagDesc elemOpts attrs = do
  _ <- char '<'
  elemTag <- mkElemtagParser elemOpts
  attrs' <- attrsParserDesc attrs
  return (elemTag, attrs')
...
svDigEq (charA:as) (charB:bs) = case (as, bs) of
  (a2:as', _) | a2 == charB -> digitEq as' bs
  (_, b2:bs') | b2 == charA -> digitEq as bs'
...
digitEqFree [] [] = True
digitEqFree (a:as) [] = if elem a ['0'..'9'] then digitEqFree as [] else False
digitEqFree [] (b:bs) = if elem b ['0'..'9'] then digitEqFree [] bs else False
digitEqFree (a:as) (b:bs)
  | elem a ['0'..'9'] = digitEqFree as (b:bs)
  | elem b ['0'..'9'] = digitEqFree (a:as) bs
  | a == b = digitEqFree as bs
  | otherwise = False
...
unfit [] _ = []
unfit ((n,v):ns) mp = case Map.lookup n mp of
  Nothing -> (n, "no attr") : unfit ns mp
  Just val -> if elem n ["href", "alt", "title"] then  unfit ns mp
              else if digitEq v val
                   then unfit ns mp
                   else (n<>":"<>"("<>val<>"|"<>v<>")", "failed test") : unfit ns mp
...
parseOpeningTag elemOpts attrsSubset = do
  ...
  case attrs of
    ...
    Right whateva -> return (elemTag, whateva)
