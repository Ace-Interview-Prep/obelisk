...
import Prelude
  ( Bool(..), Char, Either(..), Maybe(..), Show(..), String
  , Semigroup(..), Monoid(..)
  , ($), (.), (<$>), (>>), (>>=), (<*>), (<*)
  , either, fmap, snd, const, elem, id, not, null, pure, reverse
  , foldr, return
  )
...
import Text.Parsec (parse, ParsecT, Stream, string, (<|>), anyChar, char
                   , optional, try, manyTill, many, parserZero, alphaNum, oneOf
                   , digit
                   , option
                   , letter
                   , space
...
onlyPlainText = fmap (\(ACT strings) -> mconcat $ reverse strings) specialElemParser
  where
    ...
    specialElemParser = do
      (elem', _attrs') <- parseOpeningTag (Just ["html"]) []
