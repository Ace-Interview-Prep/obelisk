...
import Prelude
  ...
  , Eq(..), Num(..), Ord(..), Show(..)
  , ($), (.), (<>), (<$>), (*>), (>>=), (>>)
  , return, fmap, foldr, fst, elem, filter, flip, length
  , mempty, not, null, pure, reverse, undefined, drop
  )
...
validateGPR manyElHeads =
  if length (filter (\case {One _ -> True; _ -> False}) manyElHeads) == 0 then return []
...
htmlGenParserRepeat elemTag match manyElHeadss =
  case manyElHeadss of
    [] -> do
      ...
      if length (filter (\case {Element _ -> True; _ -> False}) htMMers) == 0
        then
        return htMMers
        else
...
treeElemParserSpecificContinuous match manyElHeads = do
  ...
  (e,at) <- parseOpeningTag (Just $ fmap (fst . rootLabel . fromMany) elSet) []
  (innerForest, outputStack) <- tryElHeads (e,at) elSet
  ...
  return $ (,) manyElHeads' (TreeHTML e at m inTx inTr)
...
innerParserContains match tag subTree =
  case elem tag selfClosing of
    ...
    False -> do
      ...
      return (matchBook, (reverse inText), (reverse treees))
