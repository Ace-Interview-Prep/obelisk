...
--
-- e.g. @getHtml' \"https:\/\/google.ca\" >>= return . runScraperOnHtml (el \"a\" [])@ would give
-- all @\<a\>@ tag HTML elements on google.ca.
--
-- @since 0.1.0.0
...
-- | Run a scraper and return only the first match (unsafe: uses 'head').
--
-- @since 0.1.0.0
runScraperOnHtml1 :: ParsecT String () Identity a -> String -> Maybe a
runScraperOnHtml1 p html = case runScraperOnHtml p html of
  Just (x:_) -> Just x
  _          -> Nothing
