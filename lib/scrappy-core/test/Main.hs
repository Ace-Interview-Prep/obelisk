...
import Hedgehog
  ( Property, property, (===), assert, failure, success
  , annotateShow
  )
...
import Text.Parsec (parse, ParsecT, ParseError, string, anyChar)

import Scrappy.Elem.ElemHeadParse
import Scrappy.Elem.ITextElemParser
import Scrappy.Elem.SimpleElemParser
  ( eitherP, el, elemParser, sameElTag, matchesInSameElTag
  , selfClosing, stylingTags, clickableHref, elemParserWhere
  )
import qualified Scrappy.Elem.SimpleElemParser as SEP (manyTill_, stylingElem)
import Scrappy.Elem.TreeElemParser
  ( Many(..), fromMany, takeTill, treeElemParser, skipManyTill
  , sameTreeH, htmlGroup, groupify, table
  )
import qualified Scrappy.Elem.TreeElemParser as TEP (selfClosing, manyTill_)
...
main :: IO ()
main = defaultMain $ testGroup "scrappy-core"
  [ typesTests
  , linksTests
  , findTests
  , elemTypesTests
...
linksTests = testGroup "Scrappy.Links"
  ...
  , testPropertyNamed "fixURL returns href with scheme as-is" "fix_url_scheme" $
      property $ fixURL (Link "https://example.com/old") "https://other.com/new" === "https://other.com/new"
  ...
  , testPropertyNamed "getFileName extracts last segment" "get_filename" $
      property $ getFileName (Link "https://example.com/foo/bar.html") === Just "bar.html"

    -- parseLink
  , testPropertyNamed "parseLink resolves relative" "parse_link_relative" $
      property $ assert $ isJust (parseLink False (Link "https://example.com/page") "/other")
  , testPropertyNamed "parseLink same-site rejects different" "parse_link_same_site_reject" $
      property $ assert $ isNothing (parseLink True (Link "https://example.com/page") "https://other.org/foo")
  , testPropertyNamed "parseLink accepts http link" "parse_link_http" $
      property $ assert $ isJust (parseLink False (Link "https://example.com/page") "https://somewhere.com/foo")

    -- urlIsNew
  , testPropertyNamed "urlIsNew true for unvisited" "url_is_new_true" $
      property $ assert $ urlIsNew ([] :: [((), String)]) "https://example.com/page"
  , testPropertyNamed "urlIsNew false for visited" "url_is_new_false" $
      property $ assert $ not $ urlIsNew [(() :: (), "https://example.com/page")] "https://example.com/page"

    -- maybeUsefulNewUrl
  , testPropertyNamed "maybeUsefulNewUrl filters visited" "useful_new_url_visited" $
      property $ assert $ isNothing $
  ...
  , testPropertyNamed "renderLink extracts url" "render_link" $
      property $ renderLink (Link "https://example.com") === "https://example.com"
...
findTests :: TestTree
findTests = testGroup "Scrappy.Find"
  [ -- findNaive
  ...
  , testPropertyNamed "findSomeHTMLNaive finds matches" "find_some_html_naive" $
      property $ findSomeHTMLNaive (string "hi") ("say hi and hi again" :: String) === Just ["hi", "hi"]
  , testPropertyNamed "findSomeHTMLNaive no match" "find_some_html_naive_none" $
      property $ findSomeHTMLNaive (string "z") ("abc" :: String) === Nothing
  ...
  , testPropertyNamed "findSomeHTML Right Nothing on no match" "find_some_html_no_match" $
      property $ findSomeHTML (string "z") ("abc" :: String) === Right Nothing
  ...
  , testPropertyNamed "streamEdit with itself" "stream_edit_self" $
      property $ streamEdit (string "hello") (\s -> s ++ "!") "say hello" === "say hello!"

    -- findSomeHTMLNaive empty
  , testPropertyNamed "findSomeHTMLNaive empty input" "find_some_html_naive_empty" $
      property $ findSomeHTMLNaive (string "z") ("" :: String) === Nothing
  ]
...
elemTypesTests = testGroup "Scrappy.Elem.Types"
  ...
  , testPropertyNamed "maxLength finds longest" "max_length" $
      property $ maxLength [[1,2], [1,2,3], [1]] === ([1,2,3] :: [Int])
  ...
  , testPropertyNamed "mkGH then ungroup roundtrip" "mkgh_ungroup" $
      property $ do
        let elems = [ Elem' "div" Map.empty ([] :: [String]) "a"
  ...
  , testPropertyNamed "longestElem empty" "longest_elem_empty" $
      property $ assert $ isNothing $ longestElem ([] :: [Elem' String])
  ...
  , testPropertyNamed "ShowHTML Char" "showhtml_char" $
      property $ showH 'a' === show 'a'
  , testPropertyNamed "ShowHTML Text" "showhtml_text" $
      property $ showH ("hello" :: T.Text) === "hello"
  ...
  , testPropertyNamed "noPat is Nothing" "nopat_nothing" $
      property $ assert $ isNothing (noPat :: Maybe (ParsecT String () Identity String))
  ...
  , testPropertyNamed "GroupHtml Ord" "grouphtml_ord" $
      property $ assert $ GroupHtml ([] :: [Elem' String]) 2 3 <= GroupHtml ([] :: [Elem' String]) 3 5
  , testPropertyNamed "GroupHtml Show empty" "grouphtml_show_empty" $
      property $ assert $ not $ null $ show (GroupHtml ([] :: [Elem' String]) 0 0)
  ...
  , testPropertyNamed "UrlPagination Eq" "url_pagination_eq" $
      property $ UrlPagination "a" "b" === UrlPagination "a" "b"
  , testPropertyNamed "UrlPagination Show" "url_pagination_show" $
      property $ assert $ not $ null $ show (UrlPagination "a" "b")

    -- AttrsError
  , testPropertyNamed "AttrsError Show" "attrs_error_show" $
      property $ show IncorrectAttrs === "IncorrectAttrs"
...
elemHeadParseTests = testGroup "Scrappy.Elem.ElemHeadParse"
  [ -- parseOpeningTag
...
simpleElemParserTests = testGroup "Scrappy.Elem.SimpleElemParser"
  [ -- eitherP
...
chainHTMLTests = testGroup "Scrappy.Elem.ChainHTML"
  [ -- nl
...
treeElemParserTests = testGroup "Scrappy.Elem.TreeElemParser"
  ...
  , testPropertyNamed "Many Show One" "many_show_one" $
      property $ assert $ not $ null $ show (One (1 :: Int))
  , testPropertyNamed "Many Show Many" "many_show_many" $
      property $ assert $ not $ null $ show (Many (2 :: Int))
  ]
...
iTextElemParserTests = testGroup "Scrappy.Elem.ITextElemParser"
  [ -- WrittenWord
    testPropertyNamed "WrittenWord Show" "ww_show" $
      property $ show (WW "hello") === "hello"
  , testPropertyNamed "WrittenWord Semigroup" "ww_semigroup" $
      property $ unWord (WW "hello" <> WW "world") === "hello world"
  , testPropertyNamed "WrittenWord Monoid identity" "ww_mempty" $
      property $ unWord (mempty :: WrittenWord) === ""
  ...
  , testPropertyNamed "Sentence Monoid identity" "sentence_mempty" $
      property $ length (unSentence (mempty :: Sentence)) === 0
  ...
  , testPropertyNamed "removeStyleTags returns tag representations" "remove_style_bold" $
      property $ assert $ removeStyleTags "<b>bold</b> text" /= "<b>bold</b> text"
  , testPropertyNamed "removeStyleTags on plain text returns empty" "remove_style_none" $
      property $ removeStyleTags "plain text" === ""

    -- catEithers
  , testPropertyNamed "catEithers extracts Rights" "cat_eithers" $
      property $ catEithers ([Left "a", Right "b", Left "c", Right "d"] :: [Either String String]) === ["b", "d"]
  , testPropertyNamed "catEithers empty" "cat_eithers_empty" $
      property $ catEithers ([] :: [Either String String]) === []
  ...
  , testPropertyNamed "Paragraph Monoid" "paragraph_monoid" $
      property $ length (unParagraph (mempty :: Paragraph)) === 0
...
scrapeTests = testGroup "Scrappy.Scrape"
  ...
  , testPropertyNamed "scrape no match" "scrape_no_match" $
      property $ assert $ isNothing $ scrape (el "span" []) "<div>no spans</div>"
  ...
  , testPropertyNamed "scrapeFirst' no match" "scrape_first_none" $
      property $ assert $ isNothing (scrapeFirst' (el "span" []) "<div>nope</div>")

    -- exists
  , testPropertyNamed "exists True on match" "exists_true" $
      property $ assert $ exists (el "p" []) "<p>hi</p>"
  , testPropertyNamed "exists False on no match" "exists_false" $
      property $ assert $ not $ exists (el "span" []) "<div>nope</div>"
  ...
  , testPropertyNamed "scrapePrefixed no match" "scrape_prefixed_none" $
      property $ assert $ isNothing (scrapePrefixed "ZZZ" (el "span" []) "no match here")
