...
import Jenga.Asset.Cabal (writeCabalProject, SimplePkg(..))
...
staticModuleFile moduleName paths = do
  ...
  return $ T.unlines
    ...
    , "static = \"static/\" <> staticPath @a" --TODO: Use jenga-route to generate this in a more consistent way
    , ""
    , T.pack $ pprint decs
    ]
